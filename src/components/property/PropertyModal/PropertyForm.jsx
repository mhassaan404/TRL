...
//   const [showNewBuilding, setShowNewBuilding] = useState(false)
//   const [newBuilding, setNewBuilding] = useState({ buildingName: '', cityId: '', typeId: '', address: '' })
//   const [savingBuilding, setSavingBuilding] = useState(false)
...
//   const saveNewBuilding = async () => {
//     if (!newBuilding.buildingName.trim()) {
//       toast.error('Building name is required')
//       return
//     }
//     if (!newBuilding.cityId || !newBuilding.typeId) {
//       toast.error('City and Type are required')
//       return
//     }
//     setSavingBuilding(true)
//     try {
//       const created = await propertyService.createBuilding({
//         buildingName: newBuilding.buildingName.trim(),
//         cityId: newBuilding.cityId,
//         typeId: newBuilding.typeId,
//         address: newBuilding.address,
//       })
//       toast.success('Building created')
//       setShowNewBuilding(false)
//       setNewBuilding({ buildingName: '', cityId: '', typeId: '', address: '' })
//       if (onBuildingCreated) await onBuildingCreated()
//       if (created?.id) {
//         setFormData((prev) => ({ ...prev, buildingId: created.id, floorId: '' }))
//         if (loadFloors) loadFloors(created.id)
...
//               boxShadow: "0 4px 12px rgba(0,0,0,0.25)"
//             }}>
//               <small className="fw-semibold d-block mb-2" style={{ color: "#F5F7FA", letterSpacing: "0.4px" }}>
//                 New Building
//               </small>
...
//                 onChange={(e) => setNewBuilding({ ...newBuilding, typeId: e.target.value })}
//               >
//                 <option value="">Select Type *</option>
//                 {buildingTypes.map((t) => (
...
const PropertyForm = ({
  ...
  useEffect(() => {
    if (editData?.id) {
      ...
      // Pre-selected building (and, when opened from the Units level, floor too)
      setFormData({
        ...defaultForm,
        buildingId: editData.buildingId,
        floorId: editData.floorId || '',
